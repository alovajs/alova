...
3. 高扩展性设计，其一，alova 的设计中大量使用了适配器模式和钩子函数，例如适配器有`requestAdapter`、`storageAdapter`等，钩子函数有`beforeRequest`、`reseponded`、`transform`、`localCache`等，而且大多存在默认行为，这样设计的目的是为了在保留高扩展性的同时，使用也足够简单；其二，全局请求参数可覆盖，例如`timeout`、`shareRequest`等，对于特别的请求可单独设置这些参数。
