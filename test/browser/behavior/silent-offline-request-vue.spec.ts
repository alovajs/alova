...
describe('send a silent request with useRequest', function () {
	test("send a silent's post", done => {
		...
		const { loading, data, downloading, error, send, onSuccess, onComplete } = useRequest(Post, {
			immediate: false,
			silent: true
		});
	...
	test("should push to localStorage when silent's post is failed", done => {
		...
		const Post = alova.Post<Result<string>>('/unit-test', { postData: 'abc' });
		const { loading, data, downloading, error, send } = useRequest(Post, { immediate: false, silent: true });
		expect(loading.value).toBeFalsy();
		expect(data.value).toBeUndefined();
		expect(downloading.value).toEqual({ total: 0, loaded: 0 });
		expect(error.value).toBeUndefined();
	...
	test('should push to localStorage instead send request when network offline', async () => {
		...
		const { loading, data, downloading, error } = useRequest(Post, { silent: true });
		expect(loading.value).toBeFalsy();
		expect(data.value).toBeUndefined();
		expect(downloading.value).toEqual({ total: 0, loaded: 0 });
		expect(error.value).toBeUndefined();
