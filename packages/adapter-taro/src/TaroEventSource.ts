...
/**
 * An EventSource-compatible implementation for the **Taro** platform.
 *
 * Taro does not expose streaming `fetch`. Instead it provides
 * `Taro.request({ enableChunked: true })` whose `onChunkReceived` callback
 * delivers the stream as `ArrayBuffer`s. This class decodes those chunks, splits
 * them into SSE lines and feeds them into the exact same event-parsing /
 * dispatch / reconnection logic as the browser `EventSourceFetch`, so `useSSE`
 * behaves identically across platforms.
 */
export default class TaroEventSource {
  static CONNECTING = 0;
  static OPEN = 1;
  static CLOSED = 2;
  url: string;
  ...
  constructor(url: string, reconnectTime: number | null = 1000, options: Record<string, any> = {}) {
    ...
    this.readyState = TaroEventSource.CONNECTING;
    this._connect();
  }
  ...
  private _connect() {
    ...
    this.readyState = TaroEventSource.OPEN;
  ...
  get open() {
    return this.readyState === TaroEventSource.OPEN;
  }

  close() {
    this._isClosed = trueValue;
    this.readyState = TaroEventSource.CLOSED;
    this._task?.abort();
  }

  private _onError(error: Error) {
    this._dispatchEvent({ type: MessageTypeError, data: '', error });
    if (this.readyState !== TaroEventSource.CLOSED) {
      this._reconnect();
    }
  }
