<script>
  ...
  let activeRoute =
    new URLSearchParams(location.search).get('path') || formatActiveRoute(routes[0].category, routes[0].items[0].title);
  let showMenu = false;
  function formatActiveRoute(category, title) {
    return `${category.toLowerCase()}.${title.toLowerCase().replace(/\s/g, '_')}`;
  }
  ...
  onMount(() => {
    window.addEventListener('popstate', () => {
      ...
      activeRoute = path || formatActiveRoute(routes[0].category, routes[0].items[0].title);
    });
  });

  function handleMenuClick(route, item) {
    const routeFlag = formatActiveRoute(route.category, item.title);
    history.pushState({}, '', `?path=${routeFlag}`);
    activeRoute = routeFlag;
    showMenu = false;
...
<div
  ...
  <div
    ...
    <div class="flex flex-row items-center">
      ...
      <nord-badge
        class="ml-2"
        style="--_n-badge-chip-color: {config.theme}; --_n-badge-border-color: {config.theme}; --_n-badge-background-color: #fff;">
        {config.pkgName}
      </nord-badge>
    </div>
        ...
        <div class="flex flex-col mb-8">
          ...
          <div class="flex flex-col border-l-[1px] border-gray-200">
            {#each route.items as item}
              <a
                on:click={() => handleMenuClick(route, item)}
                class="pl-4 border-l-[1px] -ml-[1px] hover:border-primary cursor-pointer my-2">{item.title}</a>
            {/each}
          </div>
        </div>
      {/each}
  ...
  <!-- mask -->
  <div
    class={clsx('fixed top-0 left-0 w-[100vw] h-full z-10 bg-gray-800 bg-opacity-30', showMenu ? 'block' : 'hidden')}
  ...
  <nord-toast-group
    bind:this={toastGroupRef.current}
