...
import { For, Suspense, createEffect, createMemo, createSignal, lazy, onCleanup, onMount } from 'solid-js';
...
function Layout() {
  ...
  const [activeRoute, setActiveRoute] = createSignal(path);
  ...
  const handleMenuClick = (route, item) => {
    ...
    window.history.pushState({}, '', `?path=${routeFlag}`);
  ...
  const [network, setNetwork] = createSignal(networkStatus.value);
  createEffect(() => {
  ...
  return (
    <div
      ...
      <div
        class={`fixed top-0 left-[max(0px, calc(50% - 45rem))] bg-white z-20 h-full transition-transform duration-300 flex flex-col w-80 border-r-[1px] border-gray-300 py-8 px-4 md:translate-x-0 overflow-y-auto ${showMenu() ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <div class="p-8 md:pl-96 flex flex-col flex-1">
        ...
        <div class="flex flex-col mb-6 md:flex-row md:justify-between">
          <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-2">
            <h2 class="font-bold text-2xl md:mr-4 mb-2">{activeView().title}</h2>
        ...
        <Suspense>{activeView().component()}</Suspense>
      </div>
      {/* unique toast component */}
      <nord-toast-group
        ref={el => (toastGroupRef = el)}
        autoDismiss={3000}
      />
    </div>
  );
